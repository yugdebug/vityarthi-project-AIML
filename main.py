# import all the given python libraries 
import pandas as pd
...
(x_train, y_train),(x_test,y_test)=mnist.load_data() # split the data in training set as a simple tuple

x_train = tf.keras.utils.normalize(x_train , axis = 1) # normalize i.e get all values in train from 0 to 1
x_test = tf.keras.utils.normalize(x_test , axis = 1)

#create the Ml layers 
model= tf.keras.models.Sequential()
model.add(tf.keras.layers.Flatten(input_shape=(28,28)))
model.add(tf.keras.layers.Dense(units=128,activation=tf.nn.relu))#units are the neurons of the network
...
for x in range(1,4):
    ...
    print("----------------")
    plt.imshow(img[0],cmap=plt.cm.binary) #will show the given digit and again invert it
    plt.show()
